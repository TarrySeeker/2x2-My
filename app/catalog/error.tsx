...
/**
 * Route-level error boundary для /catalog.
 * Тексты приходят из CMS (`ui_strings`, namespace `errors`/`buttons`)
 * через UiStringsProvider, подключённый в `app/layout.tsx`.
 *
 * Fallback-строки совпадают с предыдущими hardcoded значениями —
 * если БД недоступна или провайдер не успел подняться, UX не ломается.
 */
export default function CatalogError({
  ...
  const message = useUiString(
    "error.catalog.text",
    "Попробуйте обновить страницу или вернитесь на главную. Если ошибка повторится — напишите нам, мы оперативно подберём услугу вручную.",
  );
  const retryLabel = useUiString("error.catalog.cta_label", "Обновить");
  const homeLabel = useUiString("button.back_home", "На главную");
