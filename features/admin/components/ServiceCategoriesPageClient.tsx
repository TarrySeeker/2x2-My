...
} from "@/features/admin/actions/service-categories";
...
export default function ServiceCategoriesPageClient({
  ...
  async function handleDelete(id: number) {
    setDeleting(true);
    const res = await deleteServiceCategoryAction(id);
    setDeleting(false);
    if (!res.ok) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    startTransition(async () => {
      const res = await reorderServiceCategoriesAction(
  ...
  function handleSaved(saved: ServiceCategory, isCreate: boolean) {
    setCategories((prev) =>
      isCreate
        ? [...prev, saved].sort(
            (a, b) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        description="Если есть услуги, использующие эту категорию, удаление будет отклонено. Сначала переназначьте такие услуги на другую категорию."
        confirmText="Удалить"
        variant="danger"
        loading={deleting}
      />
...
function CategoryDialog({
  ...
  onClose: () => void;
  onSaved: (c: ServiceCategory, isCreate: boolean) => void;
}) {
  ...
  async function onSubmit(data: ServiceCategoryFormData) {
    if (isEdit && category) {
      const res = await updateServiceCategoryAction(category.id, data);
      if (!res.ok) {
      ...
      const res = await createServiceCategoryAction(data);
      if (!res.ok) {
        toast.error(res.error ?? "Ошибка сохранения");
        return;
      }
    ...
    >
      ...
      >
        ...
        <h3 className="text-lg font-bold text-brand-dark dark:text-white">
          {isEdit ? "Редактировать категорию" : "Новая категория услуг"}
        </h3>
        <p className="mt-1 text-sm text-neutral-500">
          Категория группирует карточки услуг на витрине и в форме услуги.
        </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-5">
          ...
          <Field
            ...
            hint="Латиница, цифры, дефисы. Используется в БД (services.category)."
          >
            ...
            {slugValue && (
              <p className="mt-1 font-mono text-[11px] text-neutral-400">
                services.category = &quot;{slugValue.toLowerCase()}&quot;
              </p>
            )}
          </Field>
          ...
          >
            <textarea
              ...
              placeholder="Короткое описание категории, отображается на витрине"
            />
          </Field>
          ...
          {isEdit && (
            <p className="rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-900 dark:bg-blue-500/5 dark:text-blue-200">
              При смене slug услуги, привязанные к старому значению,
